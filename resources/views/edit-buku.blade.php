...
<head>
    ...
    <title>JaBry | Edit Buku</title>
    @vite('resources/css/app.css')
</head>
<body class="bg-gray-100" style="height: 66rem">
    <div class="container mx-auto p-4">
        <h1 class="text-3xl font-semibold mb-7 flex justify-center mt-8">Edit Data Buku</h1>
        ...
        <form action="/buku/{{$buku->id}}" method="POST" enctype="multipart/form-data">
            ...
            <div class="space-y-4 mr-56 ml-56">
                <label for="cover" class="font-medium flex justify-center">Ubah Cover</label>
                <div class="flex justify-center mb-5">
                    
                    <input type="file" name="cover" id="cover" value="{{ asset('storage/' . $buku->id = $buku->cover) }}" class="file-input file-input-bordered file-input-info w-full max-w-xs mb-4"/>
                </div>
                <div class="flex space-x-4">
                    
                    <div class="w-1/2">
                        <label for="judul" class="block font-medium">Judul Buku</label> 
                        <input type="text" name="judul" id="judul" value="{{$buku->judul}}" class="input input-bordered w-full" placeholder="Masukkan Judul" required>
                    </div>
                    <div class="w-1/2">
                        <label for="penulis" class="block font-medium">Penulis Buku</label>
                        <input type="text" name="penulis" id="penulis" value="{{$buku->penulis}}" class="input input-bordered w-full" placeholder="Masukkan Nama Penulis">
                    </div>
                </div>
                <div>
                    <label for="deskripsi" class="block font-medium">Deskripsi</label>
                    <textarea name="deskripsi" id="deskripsi" class="textarea textarea-bordered w-full" placeholder="Masukkan Deskripsi" required>{{$buku->deskripsi}}</textarea>
                </div>
                <div class="flex space-x-4">
                    <div class="w-1/2">
                        <label for="tgl_terbit" class="block font-medium">Tanggal Terbit</label>
                        <input type="date" name="tgl_terbit" id="tgl_terbit" value="{{$buku->tgl_terbit}}" class="input input-bordered w-full" required>
                    </div>
                    <div class="w-1/2">
                        ...
                        <select name="kategori" id="kategori" class="select select-bordered w-full" required>
                            ...
                            @if ($buku->kategori == "{{$buku->kategori}}")
                            <option value="{{$buku->kategori}}">{{$buku->kategori}}</option>
                            @else
            ...
            <div class="mt-6 ml-56 mr-56">
                <button type="submit" class="btn btn-info w-full text-white">UPDATE</button>
            </div>
        </form>
        <script>

            const existingCover = "{{ asset('storage/' . $buku->cover) }}";
